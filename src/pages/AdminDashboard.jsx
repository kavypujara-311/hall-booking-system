...
const AdminDashboard = ({ onLogout }) => {
    ...
    return (
        <div className="min-h-screen bg-[#09090b] text-white font-semibold font-medium font-sans selection:bg-luxury-blue/30 overflow-hidden flex">

            {/* --- ADAPTIVE SIDE NAVIGATION --- */}
            <motion.aside
                initial={false}
                animate={{ width: isSidebarOpen ? 320 : 100 }}
                className="relative z-50 bg-[#18181b] border-r border-zinc-800 h-screen flex flex-col transition-all duration-700 ease-[0.16, 1, 0.3, 1]"
            >
                {/* Branding */}
                        ...
                        {isSidebarOpen ? (
                            ...
                            >
                                <div className="w-10 h-10 bg-luxury-blue rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                                    <Command className="w-6 h-6 text-white font-semibold font-medium" />
                                </div>
                    ...
                    {sidebarItems.map((item) => (
                        <button
                            ...
                            className={`w-full group relative flex items-center gap-4 p-4 rounded-2xl transition-all duration-500 overflow-hidden ${activeTab === item.id
                                ? 'bg-luxury-blue/10 text-white font-semibold font-medium'
                ...
                <div className="p-4 border-t border-zinc-800">
                    <button
                        ...
                        className="w-full flex items-center gap-4 p-4 rounded-2xl text-zinc-300 font-medium hover:text-white font-semibold font-medium hover:bg-zinc-800 transition-all mb-2"
                    >
            ...
            <main className="flex-1 relative h-screen overflow-y-auto overflow-x-hidden custom-scrollbar">
                ...
                <header className={`sticky top-0 z-40 px-12 py-6 flex items-center justify-between transition-all duration-500 ${scrolled ? 'bg-[#09090b]/80 backdrop-blur-xl border-b border-zinc-800 py-4' : ''}`}>
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-zinc-800 rounded-xl border border-zinc-800 group cursor-pointer hover:border-luxury-blue/40 transition-all">
                            <Search className="w-4 h-4 text-zinc-300 font-medium group-hover:text-white font-semibold font-medium transition-colors" />
                        </div>
                    ...
                    <div className="flex items-center gap-10">
                        <div className="relative group cursor-pointer">
                            <Bell className="w-5 h-5 text-zinc-300 font-medium group-hover:text-white font-semibold font-medium transition-all" />
                            <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-zinc-900" />
                        </div>
                        <div className="flex items-center gap-5 pl-10 border-l border-zinc-800">
                            <div className="text-right">
                                <p className="text-[10px] font-royal tracking-widest text-white font-semibold font-medium leading-none mb-1">{user?.name?.toUpperCase()}</p>
                                <p className="text-[8px] font-royal tracking-[0.2em] text-luxury-blue uppercase ">Sovereign Admin</p>
                            </div>
                            <div className="w-12 h-12 bg-gradient-to-br from-slate-800 to-[#09090b] rounded-2xl border border-zinc-800 flex items-center justify-center p-1 group hover:border-luxury-blue/40 transition-all cursor-pointer">
                                <div className="w-full h-full bg-[#18181b] rounded-xl flex items-center justify-center text-luxury-blue font-royal text-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {stats.map((stat, i) => (
                            <motion.div
                                ...
                                className="relative bg-[#18181b] border border-zinc-800 p-8 rounded-[3rem] group hover:border-luxury-blue/30 transition-all duration-700 overflow-hidden shadow-2xl"
                            >
                                ...
                                <div className="flex justify-between items-start mb-8">
                                    <div className="p-4 bg-zinc-800 rounded-2xl border border-zinc-800 group-hover:border-luxury-blue/20 transition-all group-hover:scale-110 duration-700">
                                ...
                                <h3 className="text-4xl font-royal text-white font-semibold font-medium group-hover:text-luxury-blue transition-colors duration-700">{stat.value}</h3>
