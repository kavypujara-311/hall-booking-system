...
const UserDashboard = ({ onLogout }) => {
    ...
    // Persist tab on refresh if coming from redirect
    ...
    const stats = [
        { label: 'TOTAL VISITS', value: bookings.length, icon: Smartphone, color: 'text-luxury-blue' },
        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-luxury-blue/30 overflow-x-hidden relative">
            ...
            <main className="relative z-10 max-w-[1800px] mx-auto px-6 lg:px-12 py-16 lg:py-24">
                ...
                >
                    ...
                    <div className="flex bg-white/[0.03] backdrop-blur-3xl p-1.5 border border-white/10 rounded-[2.5rem] shadow-2xl relative group">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                ...
                <div className="flex flex-col lg:flex-row gap-20">
                    ...
                    <aside className="lg:w-[400px] flex-shrink-0 space-y-12">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-12">
                                <div className="w-16 h-16 bg-black rounded-2xl border border-white/10 flex items-center justify-center group-hover:border-luxury-blue/40 transition-all shadow-xl">
                                    <Crown className="w-8 h-8 text-luxury-blue shadow-[0_0_15px_rgba(59,130,246,0.3)]" />
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-royal tracking-[0.3em] text-slate-500 mb-1 uppercase text-shadow">STATUS</p>
                            ...
                            <div className="space-y-8 mb-12">
                                {stats.map((stat, i) => (
                                    <div key={i} className="flex items-center justify-between p-4 bg-black/40 rounded-2xl border border-white/5 group/stat hover:border-luxury-blue/20 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className={`p-3 bg-white/5 rounded-xl border border-white/5 ${stat.color}`}>
                                                <stat.icon className="w-4 h-4" />
                                            </div>
                                            <span className="text-[9px] font-royal tracking-widest text-slate-400 group-hover/stat:text-white transition-colors">{stat.label}</span>
                                        </div>
                            ...
                            <button className="w-full py-5 bg-white text-black hover:bg-luxury-blue hover:text-white rounded-2xl font-royal tracking-[0.4em] text-[10px] font-bold transition-all duration-700 flex items-center justify-center gap-3 shadow-2xl">
                                UPGRADE PRIVILEGES <ArrowRight className="w-4 h-4" />
                            </button>
                        </motion.div>

                        {/* Event Concierge Widget */}
                        <motion.div
                            initial={{ opacity: 0, x: 30 }}
                            animate={{ opacity: 1, x: 0 }}
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 text-luxury-blue group-hover:gap-6 transition-all duration-700 cursor-pointer">
                                <span className="text-[10px] font-royal tracking-[0.3em] font-bold">ENGAGE ENVOY</span>
                                <ChevronRight className="w-4 h-4" />
                            </div>
                        </motion.div>

                        {/* Recent Protocol Feed Placeholder */}
                        <div className="px-8 border-l border-white/10 space-y-8">
                            ...
                            {[1, 2].map(i => (
                                <div key={i} className="flex gap-6 group cursor-pointer opacity-40 hover:opacity-100 transition-all duration-700">
                                    <div className="w-2 h-2 rounded-full bg-luxury-blue mt-2 shadow-[0_0_10px_rgba(59,130,246,0.8)]" />
                                    <div>
                                        <p className="text-[10px] font-royal text-white tracking-widest leading-none mb-1">SYSTEM SYNC COMPLETE</p>
                                        <p className="text-[8px] font-classic italic text-slate-500 uppercase">2 hours ago</p>
                                    </div>
                                </div>
                            ))}
                        </div>
