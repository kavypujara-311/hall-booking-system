...
const RoleCard = ({ role, title, description, icon: Icon, features, onClick, setHovered, isHovered }) => {
    ...
    const theme = {
        ...
        iconGroupText: 'group-hover:text-white',
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`
                relative h-full bg-white/5 border overflow-hidden rounded-[2.5rem] cursor-pointer transition-all duration-500 group
                ${isHovered ? `${theme.border} shadow-2xl ${theme.shadow} scale-[1.02]` : 'border-luxury-blue/10 shadow-xl'}
            `}
        >
            {/* Background Gradient on Hover */}
            <div className={`absolute inset-0 bg-gradient-to-br ${theme.bgGradient} via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
            ...
            <div className="p-10 flex flex-col h-full relative z-10">
                ...
                {/* Content */}
                <h3 className="text-3xl font-serif font-bold text-white mb-4 tracking-wide">{title}</h3>
                <p className="text-luxury-beige-200 text-lg leading-relaxed mb-10 flex-grow font-light">
                    {description}
                </p>
                ...
                <ul className="space-y-4 mb-10">
                    {features.map((feature, i) => (
                        <li key={i} className="flex items-center text-luxury-beige-100 font-light">
                ...
                {/* Button */}
                <div className="flex items-center text-white font-bold group-hover:translate-x-2 transition-transform duration-300 bg-white/5 self-start px-6 py-3 rounded-xl border border-luxury-blue/10 hover:bg-luxury-blue hover:text-black hover:border-luxury-blue text-xs uppercase tracking-widest">
