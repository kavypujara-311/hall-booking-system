...
const RecentActivityWidget = () => {
    ...
    if (loading && !activityLogs.length) {
        return (
            <div className="bg-zinc-900/50 border border-white/10 rounded-[2rem] p-6 animate-pulse mt-6">
    ...
    if (!activityLogs || activityLogs.length === 0) {
        return (
            <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="bg-zinc-900/50 border border-white/10 rounded-[2rem] p-6 text-center mt-6"
            >
    ...
    return (
        <motion.div
            ...
            className="bg-zinc-900/50 border border-white/10 rounded-[2rem] overflow-hidden backdrop-blur-md mt-6"
        >
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
                ...
                <span className="text-[10px] text-gray-500 uppercase tracking-widest">{activityLogs.length} updates</span>
            </div>
                    ...
                    {activityLogs.slice(0, 5).map((log, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="p-4 border-b border-white/5 flex gap-3 hover:bg-white/5 transition-colors items-start last:border-0"
                            >
