...
const DashboardNavbar = ({ user, onLogout, activeTab, onTabChange }) => {
    ...
    return (
        <nav className="sticky top-0 z-[60] bg-[#050505]/80 backdrop-blur-2xl border-b border-white/5 shadow-2xl transition-all duration-700">
            <div className="max-w-[1800px] mx-auto px-6 lg:px-12 h-24 flex items-center justify-between">
                ...
                <div className="flex items-center gap-16">
                    <div className="flex items-center gap-4 cursor-pointer group" onClick={() => navigate('/')}>
                        ...
                        <div className="hidden sm:block">
                            <h1 className="text-2xl font-royal font-bold tracking-[0.2em] text-white leading-none">IMPERIAL</h1>
                            <p className="text-[10px] text-luxury-blue font-royal tracking-[0.4em] leading-none mt-1 uppercase font-bold">Registry</p>
                        </div>
                    </div>

                    {/* Adaptive Tab Hub */}
                    <div className="hidden lg:flex items-center bg-white/[0.03] p-1.5 rounded-[2rem] border border-white/5 backdrop-blur-3xl shadow-inner">
                        {[
                ...
                <div className="flex items-center gap-8">
                    <div className="hidden md:flex items-center gap-4 focus-within:gap-6 transition-all duration-700 opacity-40 hover:opacity-100">
                        <Search className="w-5 h-5 text-slate-500" />
                        <span className="text-[10px] font-royal tracking-[0.3em] text-slate-500 uppercase font-bold cursor-text">Tactical Search [⌘+K]</span>
                    </div>
                    ...
                    <div className="flex items-center gap-8 pl-8 border-l border-white/10">
                        ...
                        <div className="flex items-center gap-5 group cursor-pointer" onClick={() => navigate('/profile')}>
                            <div className="text-right hidden xl:block">
                                <p className="font-royal font-bold text-white leading-none text-[11px] tracking-widest">{user?.name?.toUpperCase() || 'SOVEREIGN USER'}</p>
                                <p className="text-[8px] font-royal text-luxury-blue mt-1 tracking-[0.2em] uppercase opacity-60">Elite Member</p>
                            </div>
