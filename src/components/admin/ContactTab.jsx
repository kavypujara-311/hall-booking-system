        ...
        <div className="space-y-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-zinc-800 pb-10">
                <div>
                    ...
                    <h2 className="text-5xl font-royal text-white font-semibold font-medium leading-none">
                        CONTACT <span className="italic font-classic text-zinc-300 font-medium">Submissions</span>
                    </h2>
                </div>
                <button
                    onClick={fetchSubmissions}
                    className="flex items-center gap-3 px-8 py-4 bg-zinc-800 border border-zinc-800 rounded-2xl text-[10px] font-royal tracking-[0.3em] font-bold hover:border-luxury-blue/40 hover:text-white font-semibold font-medium text-zinc-200 font-medium transition-all"
                >
                    ...
                    <button
                        ...
                        className={`px-8 py-3 rounded-2xl text-[9px] font-royal tracking-[0.3em] font-bold border transition-all duration-500 ${
                            filter === f
                                ...
                                : 'bg-zinc-800 text-zinc-300 font-medium border-zinc-800 hover:border-white/30 hover:text-white font-semibold font-medium'
                        }`}
                    >
            ...
            {loading ? (
                ...
                <div className="py-40 text-center border border-dashed border-zinc-800 rounded-[4rem] bg-white/[0.01]">
                    ...
                    <h3 className="text-4xl font-royal text-white font-semibold font-medium mb-4">NO MESSAGES</h3>
                    <p className="text-zinc-300 font-medium font-classic italic text-lg">Your transmission log is currently empty.</p>
                </div>
            ) : (
                    ...
                    <AnimatePresence>
                        {filtered.map((s, i) => (
                            <motion.div
                                ...
                                className="bg-[#27272a] border border-zinc-800 rounded-3xl overflow-hidden hover:border-luxury-blue/20 transition-all duration-500"
                            >
                                <div 
                                ...
                                >
                                    <div className="flex flex-wrap justify-between items-center gap-6">
                                        <div className="flex-1 min-w-[300px] flex items-center gap-8">
                                            <div className="w-12 h-12 rounded-2xl bg-zinc-800 border border-zinc-800 flex items-center justify-center text-luxury-blue">
                                                <User className="w-5 h-5" />
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-4 mb-1">
                                                    <h4 className="text-sm font-royal font-bold text-white font-semibold font-medium tracking-widest">{s.name}</h4>
                                                    <span className={`px-4 py-1 rounded-full text-[8px] font-royal tracking-widest font-bold border ${STATUS_STYLES[s.status]}`}>
                                                        {s.status.toUpperCase()}
                                                    </span>
                                                </div>
                                                <p className="text-[10px] font-classic italic text-zinc-300 font-medium">{s.email} · {s.subject || 'General inquiry'}</p>
                                            </div>
                                        </div>

                                        <div className="flex items-center gap-6">
                                            <div className="text-right hidden sm:block">
                                                <p className="text-[9px] font-royal text-zinc-100 font-medium tracking-widest uppercase mb-1">RECEIVED</p>
                                                <p className="text-[10px] font-royal text-zinc-200 font-medium tracking-widest">
                                            ...
                                            <div className="flex gap-2">
                                                <button 
                                                    ...
                                                    className="p-3 bg-luxury-blue text-white font-semibold font-medium rounded-xl hover:bg-white hover:text-black transition-all flex items-center gap-2 text-[8px] font-royal tracking-widest font-bold shadow-lg"
                                                >
                                                    <ExternalLink className="w-3 h-3" /> REPLY
                                                </button>
                                                <div className="p-3 bg-zinc-800 border border-zinc-800 rounded-xl text-zinc-300 font-medium">
                                                    ...
                                                    {['read', 'replied'].map(st => (
                                                        <button 
                                                            ...
                                                            className={`px-6 py-3 rounded-xl text-[8px] font-royal tracking-widest font-bold border transition-all ${
                                                                s.status === st ? 'border-zinc-800 text-slate-700 ' : 'border-zinc-800 text-zinc-200 font-medium hover:border-white/30 hover:text-white font-semibold font-medium'
                                                            }`}
                                                        >
