        ...
        <div className="space-y-16 animate-in fade-in duration-1000">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 border-b border-zinc-800 pb-10">
                <div>
                    ...
                    <h2 className="text-5xl md:text-6xl font-royal text-white font-semibold font-medium leading-none">STRATEGIC <span className="italic font-classic text-zinc-300 font-medium">Overview</span></h2>
                </div>
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        <p className="text-[9px] font-royal tracking-[0.3em] text-zinc-300 font-medium mb-1 uppercase">SYSTEM PULSE</p>
                        <p className="text-[10px] font-royal text-emerald-500 tracking-widest font-bold">SYNCHRONIZED</p>
                    </div>
                    <div className="w-px h-10 bg-white/10" />
                    <div className="text-right text-zinc-300 font-medium font-royal text-[10px] tracking-[0.2em]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {stats.map((stat, i) => (
                    <motion.div
                        ...
                        className="bg-[#27272a] border border-zinc-800 p-10 rounded-[3.5rem] relative group overflow-hidden shadow-2xl hover:border-luxury-blue/30 transition-all duration-700"
                    >
                        ...
                        <div className="flex justify-between items-start mb-8 relative z-10">
                            <div className="p-4 bg-zinc-800 rounded-2xl border border-zinc-800 group-hover:border-luxury-blue/40 transition-all">
                        ...
                        <p className="text-[9px] font-royal tracking-[0.4em] text-zinc-300 font-medium mb-2 uppercase">{stat.label}</p>
                        <h3 className="text-4xl font-royal text-white font-semibold font-medium">{stat.value}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-8 space-y-12">
                    <div className="bg-[#27272a] border border-zinc-800 rounded-[4rem] p-12 relative overflow-hidden shadow-2xl">
                        <div className="flex justify-between items-end mb-16">
                            <div>
                                ...
                                <h3 className="text-4xl font-royal text-white font-semibold font-medium tracking-widest uppercase">Growth Matrix</h3>
                            </div>
                            <div className="flex gap-4 p-1.5 bg-zinc-800 rounded-2xl border border-zinc-800">
                                {['D', 'W', 'M', 'Y'].map(t => (
                                    <button key={t} className={`w-12 h-12 rounded-xl flex items-center justify-center font-royal text-[10px] transition-all font-bold ${t === 'M' ? 'bg-luxury-blue text-white font-semibold font-medium shadow-2xl' : 'text-zinc-300 font-medium hover:text-white font-semibold font-medium hover:bg-zinc-800'}`}>
                                        {t}
                                    </button>
                                ))}
                            </div>
                            ...
                            {chartData.map((d, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center gap-6 group relative z-10 h-full">
                                    <div className="relative w-full h-full flex items-end">
                                        ...
                                        >
                                            <div className="absolute -top-10 left-1/2 -translate-x-1/2  group-hover:opacity-100 transition-opacity bg-white text-black text-[9px] font-royal font-bold px-3 py-1.5 rounded shadow-2xl z-50 whitespace-nowrap">
                                                ₹{d.value}
                                            </div>
                                        </motion.div>
                                    </div>
                                    <span className="text-[9px] font-royal text-slate-700 group-hover:text-white font-semibold font-medium transition-colors">{d.name}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-[#27272a] border border-zinc-800 rounded-[4rem] p-12 shadow-2xl">
                        <div className="flex items-center gap-4 mb-10 border-b border-zinc-800 pb-6">
                            <PieChart className="w-5 h-5 text-luxury-blue" />
                            <h4 className="text-xl font-royal text-white font-semibold font-medium tracking-widest uppercase">ESTATE YIELD ANALYSIS</h4>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            {yields.map((y, i) => (
                                <div key={i} className="space-y-4">
                                    <div className="flex justify-between items-end">
                                        <p className="text-[10px] font-royal text-white font-semibold font-medium tracking-widest uppercase">{y.name}</p>
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-gradient-to-br from-luxury-blue/10 to-transparent border border-zinc-800 p-10 rounded-[3.5rem] relative overflow-hidden group shadow-2xl">
                        <div className="absolute top-0 right-0 p-8">
                            <ShieldCheck className="w-8 h-8 text-luxury-blue/20 animate-pulse" />
                        </div>
                        <h4 className="text-2xl font-royal text-white font-semibold font-medium mb-6 uppercase tracking-widest">System Shield</h4>
                        <div className="flex items-center gap-4 mb-4">
                        ...
                        <p className="text-[11px] font-classic italic text-zinc-300 font-medium leading-relaxed">
                    ...
                    <div className="bg-[#27272a] border border-zinc-800 p-10 rounded-[3.5rem] shadow-2xl">
                        <div className="flex items-center gap-4 mb-10 border-b border-zinc-800 pb-6">
                            <Mail className="w-5 h-5 text-luxury-blue" />
                            <h4 className="text-xl font-royal text-white font-semibold font-medium tracking-widest uppercase">RECENT INQUIRIES</h4>
                        </div>
                        <div className="space-y-8">
                            {contactSubmissions.slice(0, 4).map((s, i) => (
                                <div key={s.id || i} className="flex items-center justify-between group cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-white/[0.03] border border-zinc-800 flex items-center justify-center text-zinc-300 font-medium group-hover:text-luxury-blue transition-all">
                                            <AtSign className="w-4 h-4" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-royal text-white font-semibold font-medium tracking-widest uppercase truncate max-w-[120px]">{s.name}</p>
                                            <p className="text-[8px] font-royal tracking-widest text-zinc-300 font-medium uppercase mt-0.5">{s.status}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[9px] font-royal text-white font-semibold font-medium font-bold tracking-widest">
                            ...
                            {contactSubmissions.length === 0 && (
                                <p className="text-[10px] font-classic italic text-zinc-300 font-medium text-center py-4">No recent inquiries.</p>
                            )}
                        </div>
                    </div>
                </div>
